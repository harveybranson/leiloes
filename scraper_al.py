# -*- coding: utf-8 -*-
"""
Scraper de imoveis dos leiloeiros REGULARES de Alagoas (JUCEAL) - PDF AL.
Entrada : csv/leiloeiros_al_2026-06-09.csv (regulares; exclui CANCELADO/SUSPENSO).
...
Cache   : por site (varios leiloeiros compartilham o mesmo dominio -> raspa 1x).
Saida   : CSV datado em /csv + insercao SQLite imoveis_leiloeiros.db (dedup por url).
Report  : por leiloeiro a cada 5 min (scraper_al_progress.json + stdout).
"""
...
CSV_IN = BASE / "csv" / "leiloeiros_al_2026-06-09.csv"
OUT_DIR = BASE / "csv"
DB = BASE / "imoveis_leiloeiros.db"
PROG = BASE / "scraper_al_progress.json"
JUNTA = "JUCEAL/AL"
...
TITLE_TYPES = ["apartamento", "apto ", "casa", "terreno", "sala comercial",
               ...
               "flat", "quitinete", "sala ", "vaga de garagem", "box", "unidade"]
...
NEG_WORDS = ["veiculo", "veículo", "automovel", "automóvel", "moto ", "caminhao", "caminhão",
             ...
             "eletrodom", "movel ", "móvel ", "moveis", "móveis", "joia", "joias", "relogio", "relógio"]
...
CAT_HINT = ["imove", "imóve", "lote", "leila", "leilã", "categoria", "busca", "pesquisa", "detalhe", "judicial"]
...
_PW = None
_BROWSER = None
SITE_CACHE = {}  # site_norm -> list[imovel-dict-base] (sem o leiloeiro atribuido)
...
def site_key(site):
    try:
        return urlparse(site if "//" in site else "http://" + site).netloc.replace("www.", "").lower()
    except Exception:
        return site.lower()
...
def extract_cards(html, base):
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not same_host(href, base):
            continue
        if not re.search(r"lote|imove|imóve|detalhe|/bem/|/item|/produto|/oferta|/evento", href.lower()):
            continue
        h = a.find(["h1", "h2", "h3", "h4", "h5"]) or block.find(["h1", "h2", "h3", "h4", "h5"])
        heading = h.get_text(" ", strip=True) if h else ""
        anchor = a.get_text(" ", strip=True)
        title = best_title(href, heading, anchor)
        if not title:
            continue
        tl = title.lower()
        img = ""
        ...
        cm = re.search(r"\b([A-ZÀ-Ý][a-zà-ÿ]+(?:\s[A-ZÀ-Ý][a-zà-ÿ]+)*)\s*[/-]\s*(PE|AP|PA|AM|RR|RO|AC|MT|MS|MG|SP|RJ|ES|PR|SC|RS|BA|GO|TO|MA|PI|CE|RN|PB|AL|SE|DF)\b", btext)
...
def clean_title(t):
    ...
    # remove leading boilerplate "Leilão de/da/do ...", "Lote 12 -"
    t = re.sub(r"^(leil[ãa]o(\s+(de|da|do|dos|das|judicial|extrajudicial|on-?line|presencial|h[ií]brido))*\s+)+",
               "", t, flags=re.I)
    t = re.sub(r"^(lote\s*n?[ºo°]?\s*\d*\s*[-:–]?\s*)", "", t, flags=re.I)
    return t.strip(" -–|·:•")
...
def good_title(t):
    ...
    if len(t) < 10 or len(t) > 200:
        return False
    if not any(x in tl for x in TITLE_TYPES):
        return False
    if any(w in tl for w in NEG_WORDS) and not re.search(
            r"m²|m2|hectare|terreno|casa|apartament|im[óo]vel|sala|loja|galp[ãa]o|fazenda|s[íi]tio", tl):
        return False
    return True


def best_title(href, heading, anchor):
    cands = []
    if heading:
        cands.append(heading)
    if anchor:
        cands.append(anchor)
    # every path segment as a candidate (titles often live in the URL slug)
    for seg in urlparse(href).path.split("/"):
        seg = seg.strip()
        if seg and not seg.isdigit() and len(seg) > 8:
            cands.append(re.sub(r"[-_]+", " ", seg))
    for c in cands:
        ct = clean_title(c)
        if good_title(ct):
            return ct[:200]
...
def scrape_site(site):
    """Raspa um dominio e devolve lista de imoveis-base (sem leiloeiro). Cacheado."""
    home, base = render(site)
    if not home:
        return None  # inacessivel
    base = base or site
    ...
    pages = list(dict.fromkeys([base] + cat_links))[:6]
    ...
    for pg in pages:
        ...
        for al in collect_links(h, base, ["leilao_id", "/leilao/", "/lote", "/evento", "/oferta"]):
            if re.search(r"leilao_id|/lote|/evento|/oferta", al.lower()) and al not in pages:
...
def scrape_leiloeiro(lei):
    ...
    if key in SITE_CACHE:
        base_rows = SITE_CACHE[key]
        if base_rows is None:
            progress["status_sites"][nome] = "inacessivel (cache)"
            print("    [X] inacessivel (cache)", flush=True)
            return []
        print(f"    [cache] site ja raspado -> {len(base_rows)} imoveis", flush=True)
    else:
        base_rows = scrape_site(site)
        SITE_CACHE[key] = base_rows
        if base_rows is None:
    ...
    for b in base_rows:
        r = dict(b)
        r["leiloeiro"] = nome
        r["junta"] = lei.get("junta") or JUNTA
        r["site"] = site
        r["cidade"] = r["cidade"] or lei.get("cidade", "")
        r["uf"] = r["uf"] or lei.get("uf", "")
        imoveis.append(r)
...
def main():
    ...
    print("=" * 72)
    print(f"SCRAPER AL/JUCEAL | captura {CAPTURE:%d/%m/%Y %H:%M} | {len(leiloeiros)} leiloeiros")
    print("=" * 72, flush=True)
    ...
    if all_im:
        out = OUT_DIR / f"imoveis_al_{CAPTURE:%Y-%m-%d}.csv"
