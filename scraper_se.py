# -*- coding: utf-8 -*-
"""
Scraper de imoveis dos leiloeiros REGULARES de Sergipe (JUCESE) - PDF SE.
Entrada : csv/leiloeiros_se_2026-06-09.csv (somente Situacao=Regular; exclui IRREGULAR/CANCELADO/INATIVO).
...
Report  : por leiloeiro a cada 5 min (scraper_se_progress.json + stdout).
"""
...
CSV_IN = BASE / "csv" / "leiloeiros_se_2026-06-09.csv"
OUT_DIR = BASE / "csv"
DB = BASE / "imoveis_leiloeiros.db"
PROG = BASE / "scraper_se_progress.json"
JUNTA = "JUCESE/SE"
...
def main():
    ...
    print("=" * 72)
    print(f"SCRAPER SE/JUCESE | captura {CAPTURE:%d/%m/%Y %H:%M} | {len(leiloeiros)} leiloeiros")
    print("=" * 72, flush=True)
    ...
    if all_im:
        out = OUT_DIR / f"imoveis_se_{CAPTURE:%Y-%m-%d}.csv"
