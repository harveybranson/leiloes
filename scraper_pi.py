# -*- coding: utf-8 -*-
"""
Scraper de imoveis dos leiloeiros REGULARES do Piaui (JUCEPI) - PDF Leiloeiros Piaui.
Entrada : csv/leiloeiros_pi_2026-06-09.csv (somente regulares/aptos Portaria 002/2026;
          exclui IRREGULARES e CANCELADOS do PDF).
...
Cache   : por site (varios leiloeiros podem compartilhar o mesmo dominio -> raspa 1x).
Saida   : CSV datado em /csv + insercao SQLite imoveis_leiloeiros.db (dedup por url).
Report  : por leiloeiro a cada 5 min (scraper_pi_progress.json + stdout).
"""
...
PROG = BASE / "scraper_pi_progress.json"
JUNTA = "JUCEPI/PI"
...
def extract_cards(html, base):
    ...
    for a in soup.find_all("a", href=True):
        ...
        cm = re.search(r"\b([A-ZÀ-Ý][a-zà-ÿ]+(?:\s[A-ZÀ-Ý][a-zà-ÿ]+)*)\s*[/-]\s*(PI|PE|AP|PA|AM|RR|RO|AC|MT|MS|MG|SP|RJ|ES|PR|SC|RS|BA|GO|TO|MA|CE|RN|PB|AL|SE|DF)\b", btext)
...
def main():
    ...
    print("=" * 72)
    print(f"SCRAPER PI/JUCEPI | captura {CAPTURE:%d/%m/%Y %H:%M} | {len(leiloeiros)} leiloeiros")
    print("=" * 72, flush=True)
    ...
    if all_im:
        out = OUT_DIR / f"imoveis_pi_{CAPTURE:%Y-%m-%d}.csv"
