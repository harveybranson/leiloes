# -*- coding: utf-8 -*-
"""
Scraper de imoveis dos leiloeiros REGULARES do Maranhao (JUCEMA) - PDFs "Leiloeiros Maranhao".
Entrada : csv/leiloeiros_ma_2026-06-09.csv (28 regulares, consolidados dos 2 PDFs).
...
Report  : por leiloeiro a cada 5 min (scraper_ma_progress.json + stdout).
"""
...
CSV_IN = BASE / "csv" / "leiloeiros_ma_2026-06-09.csv"
OUT_DIR = BASE / "csv"
DB = BASE / "imoveis_leiloeiros.db"
PROG = BASE / "scraper_ma_progress.json"
JUNTA = "JUCEMA/MA"
...
_PW = None
_BROWSER = None
_DOMAIN_CACHE = {}  # dominio -> lista de imoveis (sites compartilhados: vipleiloes etc.)
...
def scrape_site(site):
    """Coleta imoveis de 1 site (com cache por dominio para sites compartilhados)."""
    dom = urlparse(site).netloc.replace("www.", "")
    if dom in _DOMAIN_CACHE:
        return _DOMAIN_CACHE[dom], True
    home, base = render(site)
    if not home:
        _DOMAIN_CACHE[dom] = None
        return None, False
    base = base or site
    ...
    for c in cards:
        ...
        if primeira <= TODAY:
            continue
        c["_primeira"] = primeira
        valid.append(c)
    _DOMAIN_CACHE[dom] = (base, valid)
    return _DOMAIN_CACHE[dom], False
...
def scrape_leiloeiro(lei):
    ...
    if not site:
        ...
        return []
    res, cached = scrape_site(site)
    if not res:
        progress["status_sites"][nome] = "inacessivel"
        progress["erros"].append(f"{nome}: inacessivel ({site})")
    ...
    for c in cards:
        imoveis.append({
            ...
            "data_leilao": c["_primeira"].strftime("%d/%m/%Y"),
            "url": c["url"], "imagem": c["imagem"], "anexos": c["anexos"],
        })
    tag = " (cache dominio)" if cached else ""
    progress["status_sites"][nome] = f"ok ({len(imoveis)}){tag}"
    print(f"    [OK] imoveis com 1a praca futura: {len(imoveis)}{tag}", flush=True)
    return imoveis
...
def main():
    ...
    print("=" * 72)
    print(f"SCRAPER MARANHAO/JUCEMA | captura {CAPTURE:%d/%m/%Y %H:%M} | {len(leiloeiros)} leiloeiros")
    print("=" * 72, flush=True)
    ...
    if all_im:
        out = OUT_DIR / f"imoveis_ma_{CAPTURE:%Y-%m-%d}.csv"
