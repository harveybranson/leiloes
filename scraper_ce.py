# -*- coding: utf-8 -*-
"""
Scraper de imoveis dos leiloeiros REGULARES de Ceara (JUCEC/TJCE/TRT7) - PDFs CE.
Entrada : csv/leiloeiros_ce_2026-06-09.csv (somente Situacao=Regular; exclui IRREGULAR/CANCELADO/INATIVO).
...
Report  : por leiloeiro a cada 5 min (scraper_ce_progress.json + stdout).
"""
...
CSV_IN = BASE / "csv" / "leiloeiros_ce_2026-06-09.csv"
OUT_DIR = BASE / "csv"
DB = BASE / "imoveis_leiloeiros.db"
PROG = BASE / "scraper_ce_progress.json"
JUNTA = "JUCEC/CE"
...
def extract_cards(html, base):
    ...
    for a in soup.find_all("a", href=True):
        ...
        if not same_host(href, base):
            continue
        if not re.search(r"lote|imove|imóve|detalhe|/bem/|/item|/produto", href.lower()):
            continue
        title = ""
...
def main():
    ...
    print("=" * 72)
    print(f"SCRAPER CE/JUCEC | captura {CAPTURE:%d/%m/%Y %H:%M} | {len(leiloeiros)} leiloeiros")
    print("=" * 72, flush=True)
    ...
    if all_im:
        out = OUT_DIR / f"imoveis_ce_{CAPTURE:%Y-%m-%d}.csv"
