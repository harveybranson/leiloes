# -*- coding: utf-8 -*-
"""
Scraper de imoveis dos leiloeiros REGULARES de Roraima/Rondonia (JUCER).
Entrada : csv/leiloeiros_roraima_rondonia_2026-06-09.csv
...
Report  : por leiloeiro a cada 5 min (scraper_rr_ro_progress.json + stdout).
"""
...
CSV_IN = BASE / "csv" / "leiloeiros_roraima_rondonia_2026-06-09.csv"
OUT_DIR = BASE / "csv"
DB = BASE / "imoveis_leiloeiros.db"
PROG = BASE / "scraper_rr_ro_progress.json"
...
# Tipo de imovel exigido NO TITULO do lote (forte) para aceitar o card
...
# palavras que indicam imovel no contexto/bloco (mais amplo)
IMOVEL_WORDS = TITLE_TYPES + ["rua ", "avenida", "av. ", "bairro", "m²", "hectare", "matricula", "matrícula"]
# produtos / veiculos / moveis -> NAO sao imoveis
...
# titulos lixo (navegacao / resumo de leilao / status)
...
# status que invalidam o lote (no bloco)
...
def extract_cards(html, base):
    ...
    soup = BeautifulSoup(html, "html.parser")
    cards = {}
    # candidatos: containers cujo texto cita imovel e que tem link
    for a in soup.find_all("a", href=True):
        block = a.find_parent(["article", "li", "div"]) or a
        btext = block.get_text(" ", strip=True)
        low = btext.lower()
        if DEAD_RE.search(low):          # lote cancelado/encerrado/arrematado
            continue
        ...
        if not re.search(r"lote|imove|imóve|detalhe|/bem/", href.lower()):
            continue
        # titulo: prioriza heading do card, depois texto do link, depois SLUG da URL
        title = ""
        ...
        # slug da URL (ex.: /lote/imovel-area-de-4896m_-morumbi-sp/238/) — muitos sites
        # SEO colocam o tipo/endereco no slug e deixam o link como imagem
        seg = [s for s in urlparse(href).path.split("/") if s and not s.isdigit()]
        slug = seg[-1] if seg else ""
        slug_txt = re.sub(r"[-_]+", " ", slug).strip()
        # se o titulo for fraco/lixo mas o slug descreve um imovel, usa o slug
        if (len(title) < 12 or JUNK_RE.search(title) or not any(t in title.lower() for t in TITLE_TYPES)) \
            ...
            title = slug_txt
        tl = title.lower()
        # rejeita lixo de navegacao / status e exige TIPO de imovel no titulo (ou slug ja aplicado)
        if (not title or len(title) < 12 or len(title) > 200 or JUNK_RE.search(title) or
                not any(t in tl for t in TITLE_TYPES)):
            continue
        # exclui produtos/veiculos (a menos que tenha medida de imovel)
        ...
        cm = re.search(r"\b([A-ZÀ-Ý][a-zà-ÿ]+(?:\s[A-ZÀ-Ý][a-zà-ÿ]+)*)\s*[/-]\s*(RO|RR|AM|MT|MG|SP|PR|SC|PA|BA|GO|TO|AC)\b", btext)
...
def db_insert(conn, rows):
    ...
    for r in rows:
        ...
        cur.execute("""INSERT INTO imoveis
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (rid, r["leiloeiro"], r.get("junta", "JUCER/RR-RO"), r["site"], r["titulo"],
...
def main():
    ...
    print("=" * 72)
    print(f"SCRAPER RR/RO | captura {CAPTURE:%d/%m/%Y %H:%M} | {len(leiloeiros)} leiloeiros")
    print("=" * 72, flush=True)
    ...
    for i, lei in enumerate(leiloeiros, 1):
        ...
        if ims:
            ...
            tot_novos += n
            tot_exist += ex
            # CSV incremental (sobrescreve com tudo)
        all_im.extend(ims)
    ...
    if all_im:
        out = OUT_DIR / f"imoveis_rr_ro_{CAPTURE:%Y-%m-%d}.csv"
        ...
        with open(out, "w", newline="", encoding="utf-8") as f:
            ...
            w.writerows(all_im)
        print(f"\n[CSV] {out}")
        importar_para_site(out, "JUCER/RR-RO")
