# -*- coding: utf-8 -*-
"""
Scraper de imoveis dos leiloeiros REGULARES de Pernambuco (JUCEPE) - PDFs PE.
Entrada : csv/leiloeiros_pe_2026-06-09.csv (regulares; exclui CANCELADA/IRREGULAR/EM ANDAMENTO).
...
Report  : por leiloeiro a cada 5 min (scraper_pe_progress.json + stdout).
"""
...
CSV_IN = BASE / "csv" / "leiloeiros_pe_2026-06-09.csv"
OUT_DIR = BASE / "csv"
DB = BASE / "imoveis_leiloeiros.db"
PROG = BASE / "scraper_pe_progress.json"
JUNTA = "JUCEPE/PE"
...
def main():
    ...
    print("=" * 72)
    print(f"SCRAPER PE/JUCEPE | captura {CAPTURE:%d/%m/%Y %H:%M} | {len(leiloeiros)} leiloeiros")
    print("=" * 72, flush=True)
    ...
    if all_im:
        out = OUT_DIR / f"imoveis_pe_{CAPTURE:%Y-%m-%d}.csv"
