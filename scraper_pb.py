# -*- coding: utf-8 -*-
"""
Scraper de imoveis dos leiloeiros REGULARES da Paraiba (JUCEP) - PDF Leiloeiros PB.
Entrada : csv/leiloeiros_pb_2026-06-09.csv (38 regulares; 6 IRREGULAR excluidos).
...
Report  : por leiloeiro a cada 5 min (scraper_pb_progress.json + stdout).
"""
...
CSV_IN = BASE / "csv" / "leiloeiros_pb_2026-06-09.csv"
OUT_DIR = BASE / "csv"
DB = BASE / "imoveis_leiloeiros.db"
PROG = BASE / "scraper_pb_progress.json"
JUNTA = "JUCEP/PB"
...
def extract_cards(html, base):
    ...
    for a in soup.find_all("a", href=True):
        ...
        cm = re.search(r"\b([A-ZÀ-Ý][a-zà-ÿ]+(?:\s[A-ZÀ-Ý][a-zà-ÿ]+)*)\s*[/-]\s*(PB|PE|AP|PA|AM|RR|RO|AC|MT|MS|MG|SP|RJ|ES|PR|SC|RS|BA|GO|TO|MA|PI|CE|RN|AL|SE|DF)\b", btext)
...
def main():
    ...
    print("=" * 72)
    print(f"SCRAPER PB/JUCEP | captura {CAPTURE:%d/%m/%Y %H:%M} | {len(leiloeiros)} leiloeiros")
    print("=" * 72, flush=True)
    ...
    if all_im:
        out = OUT_DIR / f"imoveis_pb_{CAPTURE:%Y-%m-%d}.csv"
