# -*- coding: utf-8 -*-
"""
Scraper de imoveis dos leiloeiros REGULARES do Rio Grande do Norte (JUCERN).
Entrada : csv/leiloeiros_rn_2026-06-09.csv (16 leiloeiros REGULARES; excluidos
          SUSPENSO/IRREGULAR/CANCELADO/REGISTRO SUSPENSO/cancelado a pedido).
...
Filtro  : mantem imovel cuja 1a data de leilao > data da captura; descarta lotes
          cancelados/suspensos/encerrados/arrematados (DEAD_RE).
Saida   : CSV datado em /csv + insercao SQLite imoveis_leiloeiros.db (dedup por url).
Report  : por leiloeiro a cada 5 min (scraper_rn_progress.json + stdout).
"""
...
CSV_IN = BASE / "csv" / "leiloeiros_rn_2026-06-09.csv"
OUT_DIR = BASE / "csv"
DB = BASE / "imoveis_leiloeiros.db"
PROG = BASE / "scraper_rn_progress.json"
JUNTA = "JUCERN/RN"
...
def extract_cards(html, base):
    ...
    for a in soup.find_all("a", href=True):
        ...
        low = btext.lower()
        if DEAD_RE.search(low):          # lote cancelado/encerrado/arrematado/suspenso
            continue
...
def main():
    ...
    print("=" * 72)
    print(f"SCRAPER RN/JUCERN | captura {CAPTURE:%d/%m/%Y %H:%M} | {len(leiloeiros)} leiloeiros")
    print("=" * 72, flush=True)
    ...
    if all_im:
        out = OUT_DIR / f"imoveis_rn_{CAPTURE:%Y-%m-%d}.csv"
